...
- **SwiftPM 레이아웃 — `Sources/<TargetName>` 단위 추적 필요** (Codex review 두 번째 지적): SwiftPM은 `Sources/<TargetName>/*.swift` 구조. 새 파일을 `Sources/Lib/NewFile.swift`로 추가하면 *target 디렉토리 (`Sources/Lib`)*의 mtime은 bump되지만 *상위 `Sources/`*는 변경 안 됨. manifestPaths에 `<pkg>/Sources`만 넣으면 새 파일을 silent miss. 수정: fingerprint에 *모든 inputFiles의 부모 디렉토리*를 자동 포함. 예) inputFile=`/pkg/Sources/Lib/Lib.swift` → 부모 `/pkg/Sources/Lib`도 추적 → 그 디렉토리에 sibling 추가 시 invalidation 발동. directory case도 nested 구조에서 같은 효과로 robustness 향상.
