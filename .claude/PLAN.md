...
목표: `file` + `directory` 입력 지원. 다중 파일 한 swiftc 호출.

작업:
1. `BuildInput.swift` (`file` + `directory` 케이스).
2. `BuildArgsResolver` + `LocalFilesResolver`. directory case는 디렉토리에서 `*.swift` 글롭, moduleName 자동 추정 (basename).
3. `SwiftcInvocation` 인자 다중 파일 + searchPaths/extraArgs 지원.
4. 10개 도구 마이그레이션 (input schema 변경).
5. 기존 통합 테스트 마이그레이션.
6. 새 통합 테스트: `Tests/Fixtures/MultiFileSources/{A.swift,B.swift}` 디렉토리에 대해 `emit_ast`/`compile_stats`가 두 파일의 선언을 모두 처리.

종료 조건:
- `swift build` / `swift test` 통과.
- 단일 파일 입력 + 디렉토리 입력 둘 다 동작.

#### 7.3.B — Swift 모듈 (search paths)

목표: `directory` 케이스에 `search_paths` 옵셔널 추가 → 외부 모듈 import 해석.

작업:
1. `BuildInput.directory`에 `searchPaths: [String]?` 옵셔널 필드 추가.
2. `LocalFilesResolver`가 `searchPaths`를 `-I <path>`로 매핑.
3. 통합 테스트: `Tests/Fixtures/{ModuleA,ModuleB}/`. ModuleA 미리 빌드 → ModuleB가 `import ModuleA`. `compile_stats`가 search path를 통해 import 해석.

종료 조건:
- 다른 모듈을 import하는 코드의 type-check 성공.

#### 7.3.C — SwiftPM 패키지

목표: `package` 입력. 패키지 매니페스트 + 타깃 이름으로 자동 인자 구성.

작업:
1. `BuildInput.swiftPMPackage(directory, targetName?, configuration?)` 추가.
2. `SwiftPMPackageResolver`:
   - `cd <package> && swift package describe --type json` 호출.
   - JSON 파싱: `targets[]`에서 `targetName` 일치 또는 첫 라이브러리 타깃 자동 선택.
   - 추출 정보: `path` (소스 디렉토리), `sources` (파일 리스트), `module_name`, `target_dependencies`.
   - 의존 타깃의 빌드 산출물 경로는 `swift build --build-tests`로 미리 빌드 후 `.build/<configuration>/Modules/`에서 search path 추출 (구현 시 probe로 확정).
3. `swift package describe` JSON 형식을 probe로 확정.
4. 통합 테스트: `Tests/Fixtures/SamplePackage/` (Package.swift + Sources/Lib/Lib.swift). `compile_stats` / `concurrency_audit`이 동작.

종료 조건:
- 라이브러리 타깃 1개의 패키지가 동작.
- 외부 패키지 의존 (예: swift-collections) 처리는 후속 — 일단 의존성 없는 패키지 우선.

미루는 사항: 외부 의존이 있는 패키지의 `Package.resolved`/`.build/checkouts/` 활용은 3.C 종료 후 결정.

#### 7.3.D — Xcode project

목표: `project` 입력. `xcodebuild -showBuildSettings`로 인자 구성.

작업:
1. `BuildInput.xcodeProject(path, targetName, configuration?)` 추가.
2. `XcodebuildResolver`:
   - `xcodebuild -showBuildSettings -project <path> -target <name> -configuration <cfg>` 호출.
   - 출력 `KEY = VALUE` 파싱. 관심 키: `SWIFT_INCLUDE_PATHS`, `FRAMEWORK_SEARCH_PATHS`, `OTHER_SWIFT_FLAGS`, `SDKROOT`, `ARCHS`, `MACOSX_DEPLOYMENT_TARGET` / `IPHONEOS_DEPLOYMENT_TARGET`, `SWIFT_VERSION`, `SWIFT_OPTIMIZATION_LEVEL`.
   - 입력 파일 추출: `xcodebuild -showBuildSettings`로는 알 수 없음 → `xcodebuild -project ... -target ... -dry-run -showBuildSettings` 또는 별도 query.
3. **probe 필요**: settings 출력 형식, 파일 리스트 추출 방법, target → swiftc 인자 매핑 정확도.
4. fixture: `Tests/Fixtures/SampleProject.xcodeproj/` (`project.pbxproj` + 1개 Swift 파일). `.xcodeproj`는 디렉토리 bundle이지만 텍스트 파일들로 구성되어 git commit 가능.

종료 조건:
- 1 타깃 프로젝트의 단순 분석이 동작.

미루는 사항: Build Phases 자체 파싱은 안 함 — `xcodebuild`가 처리.

#### 7.3.E — Xcode workspace

목표: `workspace` 입력. `-workspace` + `-scheme`.

작업:
1. `BuildInput.xcodeWorkspace(path, scheme, configuration?)` 추가.
2. `XcodebuildResolver` 확장: workspace 모드에서 `-workspace <path> -scheme <name>` 사용.
3. fixture: `Tests/Fixtures/SampleWorkspace.xcworkspace/contents.xcworkspacedata` + 7.3.D의 project 참조.

종료 조건:
- workspace + scheme 단순 케이스 동작.
